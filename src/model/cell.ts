...
export default class Cell extends Observable {
    ...
    setPathCost(cost: number): void {
        this.pathCost = Math.max(Math.min(this.getMaxPathCost(), cost), 1);
        this.notify();
    }
    ...
    getPathCost(raw?: boolean): number {
        return this.isWall() && !raw ? Infinity : this.pathCost;
    }
    getMaxPathCost() {
        return 20;
    }
